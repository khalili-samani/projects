...
def chart_daily_avg_price(df: pd.DataFrame, year_month: str) -> None:
    ...
    ax.set_title(f"Daily Average Wholesale Electricity Price by Region — {year_month}")
    ax.legend()
...
def chart_price_distribution_by_region(df_raw: pd.DataFrame, year_month: str) -> None:
    for region in sorted(df_raw["region_code"].unique()):
        ...
        ax.set_title(f"Electricity Price Distribution — {region} — {year_month}")
        fig.tight_layout()
...
def chart_volatility_by_region(df: pd.DataFrame, year_month: str) -> None:
    ...
    _save_chart(CHARTS_DIR / f"avg_daily_price_volatility_by_region_{year_month}.png")
...
def main(year_month: str) -> None:
    ...
    if df_daily.empty:
        logger.warning(
            "No daily summary data found for %s. Ensure the load step has been run.",
            year_month,
        )
        return
    ...
    logger.info("Analysis step completed — charts saved to %s.", CHARTS_DIR)
